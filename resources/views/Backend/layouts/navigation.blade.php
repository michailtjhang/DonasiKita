        ...
        <div class="user-panel mt-3 pb-3 mb-3 d-flex">
            <div class="image">
                @if (empty(Auth::user()->profile_photo_path))
                    <img src="{{ auth()->user()->media ? auth()->user()->media->cloudinary_url : asset('img/no_photo.svg') }}" class="img-circle elevation-2" alt="User Image">
                @else
            ...
            <div class="info">
                <a href="{{ route('profileAdmin') }}" class="d-block">{{ Auth::user()->name }}</a>
            </div>
        </div>
        <nav class="mt-2">
            @php
